...
public class LangChainAgent extends RESTService {
	...
	public Response agentResponse(@PathParam("authToken") String authToken) throws IOException {
		String response = "";
        PythonInterpreter interpreter = new PythonInterpreter();

        try {
            String openai_api_key = authToken;
            String pythonFilePath = "./etc/agent.py";
            File pythonFile = new File(pythonFilePath);
            FileReader fileReader = new FileReader(pythonFile);
            StringBuilder pythonCode = new StringBuilder();
            int i;
            while ((i = fileReader.read()) != -1) {
                pythonCode.append((char) i);
            }
            fileReader.close();

            interpreter.exec(pythonCode.toString());
            PyObject result = interpreter.eval("chat");
